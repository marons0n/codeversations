...
import { getCodingMinutesToday } from '../utils/streakManager';
import { loadEpisode, applyChoice, getStats, saveStats } from './episodeManager';

export function createGamePanel(context: vscode.ExtensionContext) {
  ...
  // Check coding time
  const minutes = getCodingMinutesToday(context);
  if (minutes < 30) {
        ...
        panel.webview.onDidReceiveMessage(
          gameMessage => {
            if (gameMessage.command === 'makeChoice') {
              ...
              stats = applyChoice(choice, stats);
              conversationIndex++;
              renderConversation();
            }
          },
...
function getWelcomeHtml(minutes: number): string {
  const pixelCss = `
...
function getGameHtml(episode: any, currentDialogue: any, stats: any, index: number): string {
...
<body>
    <div class="game-container">
        ...
        <div class="dialogue-container">
            ...
            <div class="choices">
                ${currentDialogue.options.map((option: any, i: number) => 
...
function getEffectText(effect: any): string {
  return "";
}

function getCompletionHtml(stats: any): string {
  const pixelCss = `
    <link href="https://fonts.googleapis.com/css2?family=Press+Start+2P&display=swap" rel="stylesheet">
    <style>
      :root {
